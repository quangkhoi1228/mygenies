...
export async function defaultToAccessTeamSlug(viewer: Ent<'users'>) {
  return (await viewer.edge('members').map((member) => member.edge('team').doc())).filter(
    (team) => team.deletionTime === undefined && team.isPersonal
  )[0]!.slug;
}
    ...
    return ctx.viewer
      ...
      .map(async (member) => {
        ...
        return {
          ...
          isDeleted: member.deletionTime !== undefined || team.deletionTime !== undefined,
        };
      })
...
export const create = mutation({
  ...
  async handler(ctx, { name }) {
    ...
    const teamId = await ctx.table('teams').insert({ name, isPersonal: false, slug });
