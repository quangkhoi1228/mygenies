...
import { getRole, viewerHasPermission, viewerHasPermissionX } from '../../permissions';
    ...
    const query1 =
      search === ''
        ...
        : ctx
            .table('members')
            .search('searchable', (q) =>
              q.search('searchable', normalizeStringForSearch(search)).eq('teamId', teamId)
            );
    return query1
...
export async function createMember(
  ...
  return ctx.table('members').insert({
    teamId,
    userId: user._id,
    roleId,
