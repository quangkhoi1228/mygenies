...
export async function getRole(ctx: QueryCtx, name: Role) {
  return ctx.table('roles').getX('name', name);
}

export async function viewerWithPermission(ctx: QueryCtx, teamId: Id<'teams'>, name: Permission) {
...
export async function viewerHasPermission(ctx: QueryCtx, teamId: Id<'teams'>, name: Permission) {
