...
import CustomCategoryTopic from "@/components/topic/custom-category-topic";
...
import createNewTopic from "./new-topic/helpers/createNewTopic";
...
export default function CustomizationPage() {
  ...
  const handleNewConversation = (item: ConversationCustom) => {
    ...
    const fetchData = async () => {
      ...
      router.push(`/chat/topic?id=${res.id}&ref=/let-talk/customization`);
    };
    fetchData();
  };
  const handleContinueConversation = (item: ConversationCustom) => {
    dispatch(setAudioPlayerUrl("/audio/noti_start.mp3"));
    dispatch(startPlayer());
    router.push(`/chat/topic?id=${item.id}&ref=/let-talk/customization`);
  };

  return (
    <Box height="100%">
      <CustomCategoryTopic onClick={handleOpenCustomTopic} />
