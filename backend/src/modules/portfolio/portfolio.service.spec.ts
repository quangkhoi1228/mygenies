...
describe('StockOrderService', () => {
  ...
  const mockPaginationData: FindResponseDto<Portfolio> = {
    data: [mockData],
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PortfolioService,
        { provide: getRepositoryToken(Portfolio), useValue: mockRepo },
      ],
    }).compile();

    service = module.get<PortfolioService>(PortfolioService);
    repo = module.get<Repository<Portfolio>>(getRepositoryToken(Portfolio));
  });
